...
def write_xml_file(path, data):
    try:
        ...
        print(f"[OK] Dane zapisane do pliku XML: {path}")

    except Exception as e:
        print(f"[BŁĄD] Nie udało się zapisać pliku XML: {e}")
        sys.exit(1)

def main():
    if len(sys.argv) != 3:
        print("Użycie: python Task7.py pathFile1.x pathFile2.y")
        sys.exit(1)
    ...
    try:
        ...
        elif output_format == "xml" and data is not None:
